...
try {
    ...
    foreach ($rows as $row) {
        ...
        if ($url === '' || isset($seenPartnerUrls[$url])) { continue; }
        $seenPartnerUrls[$url] = true;
        $partnerLinks[] = $row;
    }
} catch (Throwable) { $partnerLinks = []; }
...
try {
    ...
    try {
        ...
        $textRssSiteCount = $stmt ? (int)$stmt->fetchColumn() : null;
    } catch (Throwable) { $textRssSiteCount = null; }
}
...
if ($fixedPages === []) { $fixedPages = $defaultFixedPages; }
?>
<aside class="sidebar site-sidebar">
    ...
    <section class="sidebar-block">
        ...
        <?php else: ?>
            <ul class="sidebar-links sidebar-links--pages">
                <?php if ($sitePostCount !== null): ?><li><a style="color:#000;">公開商品数：<strong><?= e(number_format($sitePostCount)) ?></strong></a></li><?php endif; ?>
                <?php foreach ($fixedPages as $page): ?>
                    <?php $pageHref = trim((string)($page['href'] ?? '')); ?>
                    <?php
                    if ($pageHref === '') {
                        $pageSlug = (string)$page['slug'];
                        if ($pageSlug === CONTACT_PAGE_OLD_SLUG) { $pageSlug = CONTACT_PAGE_SLUG; }
    ...
    <section class="sidebar-block sidebar-block--ad1 only-pc"><div class="site-ad site-ad--rectangle"><?php render_ad('sidebar_bottom', $pageType, 'pc'); ?></div></section>
    <?php endif; ?>

    <?php if (site_setting_get('link.rss_display.pc_image', '1') === '1'): ?>
    <section class="sidebar-block"><?php include __DIR__ . '/rss_image_widget.php'; ?></section>
    <?php endif; ?>
    ...
    <section class="sidebar-block sidebar-block--text-rss">
        ...
        if ($textRssSiteCount !== null) { $GLOBALS['pcf_rss_widget_max_items'] = min(50, max(0, $textRssSiteCount * 5)); } else { unset($GLOBALS['pcf_rss_widget_max_items']); }
        include __DIR__ . '/rss_text_widget.php';
        if ($prevTextRssUsedKeys === null) { unset($GLOBALS['pcf_rss_widget_used_keys']); } else { $GLOBALS['pcf_rss_widget_used_keys'] = $prevTextRssUsedKeys; }
        if ($prevTextRssMaxItems === null) { unset($GLOBALS['pcf_rss_widget_max_items']); } else { $GLOBALS['pcf_rss_widget_max_items'] = $prevTextRssMaxItems; }
        ?>
    </section>
    <?php endif; ?>

    <?php if ($canRenderAd && (!function_exists('should_show_ad') || should_show_ad('content_bottom', $pageType, 'pc'))): ?>
    <section class="sidebar-block sidebar-block--ad2 only-pc"><div class="site-ad site-ad--rectangle"><?php render_ad('content_bottom', $pageType, 'pc'); ?></div></section>
    <?php endif; ?>
    ...
    <section class="sidebar-block">
        ...
        <?php else : ?>
            <ul class="sidebar-links sidebar-links--partners">
                <?php foreach ($partnerLinks as $link) : ?><li><a href="<?= e((string)$link['url']) ?>" target="_blank" rel="noopener noreferrer"><?= e((string)$link['name']) ?></a></li><?php endforeach; ?>
            </ul>
        <?php endif; ?>
    </section>
    <?php endif; ?>
